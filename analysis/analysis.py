...
def run_U_analysis(temp, run):
    ...
    for i in tqdm(range(0, len(steps-bs), bs)):
        ...
        for sim_fldr in os.listdir(path):
            ...
            ql, qs, up, ub = q_l_s(path, sim_num, temp, steps[i:i+bs])
            up_j.append(up)
            ub_j.append(ub)
            q_l_j.append(ql)
            q_s_j.append(qs)
        ...
        q_s_j = np.array(q_s_j)
        Up.append(np.mean(up_j))
        dUp.append(np.std(up_j)/np.sqrt(len(up_j) - 1))
        Ub.append(np.mean(ub_j))
        dUb.append(np.std(ub_j)/np.sqrt(len(ub_j) - 1))
        U_q.append(3/T * np.mean(q_l_j - q_s_j))
        dU_q.append(3/T * np.std(q_l_j - q_s_j)/np.sqrt(len(q_l_j) - 1))
        mc_step.append(np.mean(steps[i:i+bs]))
    plt.errorbar(mc_step, Up, yerr=dUp, capsize=2, label="Up", fmt='o')
    plt.errorbar(mc_step, Ub, yerr=dUb, capsize=2, label="Ub", fmt='o')
    plt.errorbar(mc_step, U_q, yerr=dU_q, capsize=2, label="$U(q_l, q_s)$", fmt='o')
